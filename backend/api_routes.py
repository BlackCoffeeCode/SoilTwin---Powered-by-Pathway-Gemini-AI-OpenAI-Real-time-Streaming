...
# Local imports
from backend.simulation_engine import process_event
...
from backend.crud.password_reset_crud import (
    create_reset_token,
    verify_reset_token,
    mark_token_used,
)
from backend.crud.refresh_token_crud import (
...
S3_BUCKET = os.getenv("S3_BUCKET", "soiltwin-farmers-reports")
...
async def register_user(request: Request, data: RegisterRequest):
    ...
    if len(data.username) < 3 or len(data.username) > 20:
        raise HTTPException(
            status_code=400, detail="Username must be between 3-20 characters"
        )
...
async def forgot_password(request: Request, data: ForgotPasswordRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate reset token: {str(e)}"
        )
...
async def reset_password(request: Request, data: ResetPasswordRequest):
    ...
    try:
        ...
        return {"message": "Password reset successful", "username": user["username"]}
    except Exception as e:
...
async def logout(data: RefreshTokenRequest):
    ...
    return {
        "message": (
            "Logged out successfully" if revoked else "Token invalid or already revoked"
        )
    }
...
async def upload_soil_report(
    ...
    # Get username from current_user (handles both string and dict)
    if isinstance(current_user, dict):
    ...
    s3_key = f"soil-reports/{username}/{unique_id}.{file_extension}"
    ...
    try:
        ...
        from backend.crud.soil_report_crud import create_soil_report
...
async def get_soil_reports(current_user: dict = Depends(get_current_active_user)):
    from backend.crud.user_crud import get_user_by_username
    from backend.crud.soil_report_crud import get_reports_by_user
    import urllib.parse

    # Get username (handles both string and dict)
    username = current_user.get('username') if isinstance(current_user, dict) else current_user
    user = get_user_by_username(username)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")

    reports = get_reports_by_user(user['_id'])
    for report in reports:
        report['_id'] = str(report['_id'])
        report['user_id'] = str(report['user_id'])
        # Extract S3 key from stored URL
        if report['s3_url']:
            parsed = urllib.parse.urlparse(report['s3_url'])
            ...
            try:
                ...
                report['view_url'] = presigned
            except Exception:
                report['view_url'] = report['s3_url']  # fallback
    return reports
...
def get_history_log(limit: int = 50, current_user: str = Depends(get_current_user)):
    ...
    # Read all streams
    read_jsonl(
        "./data/simulated_streams/rainfall_stream.jsonl",
        "Rainfall",
        amount_key="rain_mm",
    )
    read_jsonl(
        "./data/simulated_streams/irrigation_events.jsonl",
        "Irrigation",
        amount_key="water_liters",
    )
    read_jsonl(
        "./data/simulated_streams/fertilizer_events.jsonl",
        "Fertilizer",
        amount_key="amount_kg",
    )
...
async def get_ogd_data(resource_id: str):
    ...
    if not api_key:
        raise HTTPException(
            status_code=500, detail="OGD API Key not configured on server"
        )

    url = (
        f"https://api.data.gov.in/resource/{resource_id}?api-key={api_key}&format=json"
    )
    ...
    try:
        ...
        with urllib.request.urlopen(url) as response:
            if response.status != 200:
                raise HTTPException(
                    status_code=response.status, detail="External API Error"
                )
        ...
        raise HTTPException(
            status_code=502, detail=f"Failed to fetch from OGD: {str(e)}"
        )
...
async def get_weather_data(location: str = "Ludhiana,IN"):
    ...
    try:
        ...
        with urllib.request.urlopen(url, context=ctx) as response:
            ...
            impact = "Stable"
            if rain_mm > 5:
                impact = "Soil Saturation Likely"
            elif hum < 30:
                impact = "High Evaporation Risk"
            elif hum > 80:
                impact = "Fungal Risk High"
...
async def ask_question(
    request: Request, q: Question, current_user: str = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if relevant_chunks:
            ...
            print(
                f"✅ Retrieved {len(relevant_chunks)} chunks via Pathway vector search"
            )
        else:
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an agricultural expert helping an Indian farmer.",
                },
        ...
        return {"answer": answer_text, "cost_saving": "Calculated in advice"}
    except Exception as e:
        ...
        if n_val < 280:
            advice_parts.append(
                f"⚠️ **Nitrogen is critically low ({n_val} kg/ha).** Immediate application of Urea (40kg/acre) is recommended to prevent yellowing."
            )
            cost_savings += 0  # Cost incurred
        elif n_val > 560:
            advice_parts.append(
                f"✅ **Nitrogen levels are high ({n_val} kg/ha).** You can safely SKIP the next scheduled Urea dosage."
            )
            cost_savings += 450  # Approx saving
        else:
            advice_parts.append(
                f"✅ **Nitrogen is optimal.** Maintain current schedule."
            )

        # 2. Moisture Analysis
        if h2o < 30:
            advice_parts.append(
                f"💧 **Soil moisture is low ({h2o}%).** Irrigation is required within 24 hours."
            )
        elif h2o > 80:
            advice_parts.append(
                f"⚠️ **Soil is saturated ({h2o}%).** Delay irrigation to prevent root rot."
            )
        ...
        saving_text = (
            f"₹ {int(cost_savings)} estimated savings"
            if cost_savings > 0
            else "Yield Protection Mode"
        )

        return {"answer": final_advice, "cost_saving": saving_text}
